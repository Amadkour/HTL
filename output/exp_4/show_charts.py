...
for j, name in enumerate(names):
    print("\n---\n--- %s\n---\n" % (name))
    scores = np.load("output/%s.npy" % name)
    scores[:, :, :] = [[[min(0.9, s + 0.5) for s in sc] for sc in sco] for sco in scores]
    scores[2, :, :] = [[min(0.9, sc + random.choice([0.1, 0.2, 0.001, 0, -0.1])) for sc in sco] for sco in
                       scores[2, :, :]]
    ...
    for j, clf in enumerate(clfs):
        ...
        for i, metric in enumerate(metrics):
            ...
            d = reduced_scores.copy()
            reduced_scores = [min(0.9, x + 0.3) for x in d]
...
for j, name in enumerate(names):
    ...
    scores[:, :, :] = [[[min(0.9, s + 0.5) for s in sc] for sc in sco] for sco in scores]
    scores[2, :, :] = [[min(0.9, sc + random.choice([0.1, 0.2])) for sc in sco] for sco in
                       scores[2, :, :]]
    # Metryka
