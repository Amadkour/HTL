...
flows = [
    'Online Learning',
    'Detector Base Learning',
]
statistics = [{'time': 0, 'number_of_updates': 0} for _ in flows]
# concept_drift_method = ADWIN()
concept_drift_method = EDDM()
# streams = helper.synthetic_streams_10(123)
streams1 = helper.realstreams2()
streams2 = helper.synthetic_streams_10(123)
streams3 = helper.synthetic_streams_8(322)

scores = np.zeros((len(flows), streams1[list(streams1.keys())[0]].n_chunks - 1, len(metrics)))
sources_domain = [
    streams1[list(streams1.keys())[0]].get_chunk(),
    streams1[list(streams1.keys())[0]].get_chunk(),
    # streams1[list(streams1.keys())[0]].get_chunk(),
    streams2[list(streams2.keys())[0]].get_chunk(),
    streams2[list(streams2.keys())[0]].get_chunk(),
    # streams2[list(streams2.keys())[0]].get_chunk(),
    streams3[list(streams3.keys())[0]].get_chunk(),
    streams3[list(streams3.keys())[0]].get_chunk(),
    # streams3[list(streams3.keys())[0]].get_chunk(),
]
...
def stream_flow(flow_index, flow):
    global statistics, sources_domain, source_weights, weights, source_classifiers
    weights = []
    ...
    while True:
        ...
        if index == 0:
            ...
            if flow == flows[0]:
                ...
                time = 0
                for y in yhat:
                    drift, _ = concept_drift_method.update(y)
